...
close all
clear all
pos_l = [1; 0]; % Leader Position
pos_f = [5; 1]; % Follower Position
r = 2; % Range

%% GROUP 1: IF LEADER AND FOLLOWER POSITIONS ARE KNOWN
%% VIA NUMERICAL OPTIMIZATION
tic
f = @(x)Criterion(x, pos_l, r);
constr = @(x)Constrains(x, pos_l, r);
[x, fval] = fmincon(f, pos_f, [], [], [], [], [], [], constr);
toc
...
function f = Criterion(pos_f, pos_l, r)
    f = sqrt( (pos_f(1) - pos_l(1))^2 + (pos_f(2) - pos_l(2))^2 ) - r;
end

function [c, ceq] = Constrains(pos_f, pos_l, r)
