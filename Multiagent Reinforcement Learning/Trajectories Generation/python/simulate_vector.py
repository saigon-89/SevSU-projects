...
for iteration in range(100):
    ...
    goals = []
    obstacles = []
    while (len(agents) < AGENTS_AMOUNT):
        p = Point(random.randint(0, MAP_SIZE-1), random.randint(0, MAP_SIZE-1))
        if p not in agents and p not in goals:
            agents.append(p)
    while (len(goals) < AGENTS_AMOUNT):
        p = Point(random.randint(0, MAP_SIZE-1), random.randint(0, MAP_SIZE-1))
        if p not in agents and p not in goals:
            goals.append(p)
    while (len(obstacles) < OBSTACLES_AMOUNT):
        p = Point(random.randint(0, MAP_SIZE-1), random.randint(0, MAP_SIZE-1))
        if p not in agents and p not in goals:
            obstacles.append(p)
        ...
        for i in range(50):
            for agent, goal in zip(agents, goals):
                if calc_distance(agent, goal) == 0:
                    continue
                agents_and_obstacles = obstacles + agents
                agents_and_obstacles.remove(agent)
                ...
                obstacles_state, dto_state = calc_obstacles_state(agent, goal, agents_and_obstacles, max_possible_distance, OBSTACLE_DISTANCE_STATE_SIZE)
                close_obstacles_state = calc_close_obstacles_state(agent, goal, agents_and_obstacles)
                ...
                action(point=agent)
                if agent in agents_and_obstacles:
